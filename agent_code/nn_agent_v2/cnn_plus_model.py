...
class DoubleCNNPlusModel(nn.Module):
    """
    ...
    """
    def __init__(self):
        super(DoubleCNNPlusModel, self).__init__()
        if configs.LOAD:
            print("LOAD MODEL")
    ...
    def predict_policy(self, state: np.ndarray) -> np.ndarray:
        x_img = state.astype(np.float32)[:, 0:5, :]
        x_vec = state.astype(np.float32)[:, 5, 0, 0].reshape(-1,1)

        self.policy_net.eval()
        return self.policy_net(x_img=torch.Tensor(x_img).to(device), x_vec=torch.Tensor(x_vec).to(device)).detach().cpu().numpy()

    def predict_target(self, state: np.ndarray) -> np.ndarray:
        x_img = state.astype(np.float32)[:, 0:5, :]
        x_vec = state.astype(np.float32)[:, 5, 0, 0].reshape(-1,1)
        self.target_net.eval()
        return self.target_net(x_img=torch.Tensor(x_img).to(device), x_vec=torch.Tensor(x_vec).to(device)).detach().cpu().numpy()
    ...
    def fit(self, input_data: np.ndarray, target: np.ndarray) -> float:
        ...
        for batch, (x_img, x_vec, y) in enumerate(dataloader):
            x_img, x_vec, y = x_img.to(device), x_vec.to(device).reshape(-1,1), y.to(device)

            # Compute prediction error
            pred = self.policy_net(x_img=x_img, x_vec=x_vec)   # forward pass
...
class CNN(nn.Module):
    def __init__(self, input_channel=5):
        ...
        self.mlp = MLP(input_size=288 + 1)

    def forward(self, x_img: torch.Tensor, x_vec: torch.Tensor) -> torch.Tensor:
        """
        Forward pass of the neural network.
        Args:
            x_img: [batch_size, input_channel, height, width] tensor
            x_vec: [batch_size, feature_dim] tensor

        Returns: [batch_size, output_size] tensor
        """
        x = self.cnn_layer(x_img)
        x = self.flatten_layer(x)
        x = torch.cat((x, x_vec), dim=1)  # concat additional info to Tensors and feed into MLP
        x = self.mlp(x)
        return x
...
class StateValueDataset(Dataset):
    def __init__(self, states: np.ndarray, values: np.ndarray):
        self.input_data_one = states.astype(np.float32)[:,0:5,:]
        self.input_data_two = states.astype(np.float32)[:, 5, 0, 0]
        self.target = values.astype(np.float32)

    def __len__(self):
        return len(self.input_data_one)

    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()

        return self.input_data_one[idx], self.input_data_two[idx], self.target[idx],
