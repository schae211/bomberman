...
import events as e
from agent_code.nn_test.callbacks import state_to_features, coin_bfs, save_bfs, get_bomb_map
from agent_code.nn_test.config import configs, feature_specs, reward_specs, auxiliary_rewards, SAVE_KEY, SAVE_TIME
...
def game_events_occurred(self, old_game_state: dict, self_action: str, new_game_state: dict, events: List[str]):
    ...
    if old_game_state:
        ...
        self.memory.append(Transition(old_game_state["round"],              # round
                                      ...
                                      torch.Tensor([ACTION_TRANSLATE[self_action]]).type(torch.int64),                          # action
                                      state_to_features(new_game_state),    # next_state
                                      torch.Tensor([rewards]).type(torch.int64)))                             # reward
...
def end_of_round(self, last_game_state: dict, last_action: str, events: List[str]):
    ...
    self.memory.append(Transition(last_game_state["round"],
                                  ...
                                  torch.Tensor([ACTION_TRANSLATE[last_action]]).type(torch.int64) ,
                                  None,
                                  torch.Tensor([reward_from_events(self, events, last_game_state, None)]).type(torch.int64) ))
...
def compute_TS(self, batch):
    ...
    for ts_index, i in enumerate(batch):
        ...
        episode, state, action = self.memory[i].round, self.memory[i].state, self.memory[i].action
