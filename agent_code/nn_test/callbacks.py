...
from agent_code.nn_test.cnn_new import TestModel
...
def setup(self):
    ...
    """
    if configs.AGENT == "CNN": self.model = TestModel()
...
def act(self, game_state: dict) -> str:
    ...
    if self.train and np.random.rand() <= max(self.epsilon_min, self.epsilon * self.epsilon_reduction ** episode_n-configs.PRETRAIN_LEN):
        ...
        with torch.no_grad():
            return ACTION_TRANSLATE_REV[int(self.model.predict_policy(features).max(1)[1].view(1, 1))]
...
def state_to_features(game_state: dict) -> np.array:
    ...
    *This is not a required function, but an idea to structure your code.*
    ...
    if configs.FEATURE_ENGINEERING == "channels":
        ...
        return torch.from_numpy(stacked_channels[None,:]).type(torch.float32)
